...
from rl4co.models.zoo.matnetcaps.policy import MatNetCapsPolicy
...
def select_matnet_policy(env, **policy_params):
    if env.name == "ffsp":
        log.error(f"{env.name} not supported")
    else:
        return MatNetCapsPolicy(env_name=env.name, **policy_params)
...
class MatNet(POMO):
    def __init__(
        self,
        env: RL4COEnvBase,
        policy: Union[nn.Module, MatNetCapsPolicy] = None,
        num_starts: int = None,
        policy_params: dict = {},
        **kwargs,
    ):
